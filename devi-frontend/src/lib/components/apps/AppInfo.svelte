...
<section class="flex flex-col gap-2">
	<header class="flex items-center justify-between gap-3">
		...
		<div class="flex items-center gap-5">
			<button class="text-xl hover:underline cursor-pointer" on:click={() => editInfo(true)}>Modify</button>
			<button class="{getStatusColor(app.status)} rounded-md p-1">{app.status}</button>
		</div>
	</header>
	<section class="flex justify-between gap-10 border-b border-gray-200 p-4 px-0">
		<div class="group flex flex-1 flex-col items-center justify-center gap-3">
			<img
				src={app.logoUrl || '/no-image.png'}
				alt={app.name}
				class="h-40 w-40 rounded-md bg-white/10 transition-opacity duration-300 group-hover:opacity-50"
			/>
			<span
		...
		<div class="flex grow flex-col items-start gap-2 rounded-sm bg-gradient-to-b from-transparent to-light/20 border-b p-2">
